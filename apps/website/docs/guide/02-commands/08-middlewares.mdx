...
For hierarchical command trees, "current directory" means the
directory that contains the leaf definition:

- a shorthand leaf such as `add.subcommand.ts` uses middleware from the
  directory that contains that file
- a folder leaf such as `[archive]/command.ts` uses middleware from the
  `[archive]` directory
- parent command and group directories do not automatically contribute
  `+middleware.ts` files to deeper leaves
